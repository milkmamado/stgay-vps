...
## ⚠️ 시총·유동성 절대 원칙 (위반 금지):
liquidity 데이터를 반드시 활용하여 시장 국면(불장/하락장)에 맞춘 추천을 할 것.

**시총 등급별 가이드:**
- `junk_risk` (시총 < 500억): ⚠️ 잡주 위험. 추천 시 reasons에 "분할 매수 필수, 호가창 확인" 명시. 후순위.
- `small_volatile` (500~1000억): 📊 고변동 소형주. 변동성 큼.
- `sweet_spot` (1000~5000억): 🎯 **스윙 최적 구간** — 세력 진입 가능 + 충분한 유동성. **하락장에서도 움직임 발생, 우선 추천**.
- `mid_cap` (5000억~3조): 📈 안정적이나 지수 추종. 무난.
- `mega_cap` (3조+): 🐘 대형주 — **불장에서만 움직임. 하락장이면 후순위**.

**선정 시 우선순위:**
1. 같은 점수면 sweet_spot > small_volatile > mid_cap > mega_cap > junk_risk 순
2. 5일평균 거래대금 < 30억 종목은 reasons에 "저유동 — 호가 확인" 명시
3. liquidity.market_cap_label 그대로 reasons 또는 risks에 인용 가능
